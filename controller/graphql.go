			...
			*/
			"user": &graphql.Field{
				...
				Resolve: func(p graphql.ResolveParams) (interface{}, error) {
					...
					if idOK {
						result = service.GetUserAllInfoByID(uint(id))
					} else if nameOK {
						result = service.GetUserAllInfoByName(name)
					} else {
			...
			"group": &graphql.Field{
				...
				Resolve: func(p graphql.ResolveParams) (interface{}, error) {
					...
					var result *model.ApiJson
					if idOK {
						result = service.GetGroupAllInfoByID(uint(id))
					} else if nameOK {
						result = service.GetGroupAllInfoByName(name)
					} else {
						return nil, fmt.Errorf("No query key specified")
					}
					if !result.Status {
			...
			"permission": &graphql.Field{
				...
				Resolve: func(p graphql.ResolveParams) (interface{}, error) {
					...
					var result *model.ApiJson
					if idOK {
						result = service.GetPermissionByID(uint(id))
					} else if nameOK {
						result = service.GetPermissionByName(name)
					} else {
						return nil, fmt.Errorf("No query key specified")
					}
					if !result.Status {
